function signalParams = dtagcorrelate_noise_only_2(savedSignals,varargin)
% MS 2017.07.07
% MODIFIED TO CROSS_CORRELATE NOISE SIGNALS BEFORE AND AFTER CALLS
% MODIFIED TO ELIMINATE EXTRANEOUS FIGURES / PARAMETER CALCULATIONS
...
for i = 1:size(savedSignals,1)
    ...
    O_pre = savedSignals{i,1}.noise_pre; % Grab pre audio noise!
    A_pre = savedSignals{i,2}.noise_pre; % Grab pre accel noise!
    O_pre = decimate(O_pre, 2); % Decimate pre audio noise to accel fs!
    
    O_post = savedSignals{i,1}.noise_post; % Grab post audio noise!
    A_post = savedSignals{i,2}.noise_post; % Grab post accel noise!
    O_post = decimate(O_post, 2); % Decimate post audio noise to accel fs!
    ...
    pre_sigs{4} = A_pre(:,3);
    post_sigs{1} = O_post;
    post_sigs{2} = A_post(:,1);
    post_sigs{3} = A_post(:,2);
    post_sigs{4} = A_post(:,3);
    ...
    for row = 1:4
        for col = 1:4
            tmp_sigRow = pre_sigs{row};
            tmp_sigCol = post_sigs{col};
    ...
    for row = 1:4
        for col = 1:4
            MS_annotate_axes(ax(row,col+3),['coef =',num2str(sigs_coef{row,col}),'  delay = ',num2str(sigs_lags{row,col})])
        end
    end
...
%     MS_waveform(ax(1,1),[1:length(O_pre)]/savedSignals{i,1}.fs_dec,O_pre); %MS_waveform(ax(1,1),savedSignals{i,1}.T,y);
%     MS_waveform(ax(2,1),[1:length(A_pre)]/savedSignals{i,1}.fs_dec,A_pre(:,1)); %MS_waveform(ax(2,1),savedSignals{i,2}.T,A(:,1));
%     MS_waveform(ax(3,1),[1:length(A_pre)]/savedSignals{i,1}.fs_dec,A_pre(:,2)); %MS_waveform(ax(3,1),savedSignals{i,2}.T,A(:,2));
%     MS_waveform(ax(4,1),[1:length(A_pre)]/savedSignals{i,1}.fs_dec,A_pre(:,3)); %MS_waveform(ax(4,1),savedSignals{i,2}.T,A(:,3));
%     
%     % Plot all signal spectrograms
%     [S_aud,F_aud,T_aud] = MS_spectrogram(ax(1,2),O_pre,savedSignals{i,1}.fs_dec,pt_aud,ovl,savedSignals{i,2}.bandpass,MAP);
%     MS_spectrogram(ax(2,2),A_pre(:,1),savedSignals{i,2}.fs,pt_acc,ovl,savedSignals{i,2}.bandpass,MAP);
%     MS_spectrogram(ax(3,2),A_pre(:,2),savedSignals{i,2}.fs,pt_acc,ovl,savedSignals{i,2}.bandpass,MAP);
%     MS_spectrogram(ax(4,2),A_pre(:,3),savedSignals{i,2}.fs,pt_acc,ovl,savedSignals{i,2}.bandpass,MAP);
%     
%     % Plot all signal power spectra
%     noiseIdx = 0.15*savedSignals{i,1}.fs_dec;
%     MS_powerspectrum_noise_correction(ax(1,3),O_pre,O_pre([1:noiseIdx,end-noiseIdx:end]),savedSignals{i,1}.fs_dec,pt_aud,ovl,FLIM_AUDIO,MAP);
%     MS_powerspectrum_noise_correction(ax(2,3),A_pre(:,1),A_pre([1:noiseIdx,end-noiseIdx:end],1),savedSignals{i,2}.fs,pt_acc,ovl,FLIM_AUDIO);
%     MS_powerspectrum_noise_correction(ax(3,3),A_pre(:,2),A_pre([1:noiseIdx,end-noiseIdx:end],2),savedSignals{i,2}.fs,pt_acc,ovl,FLIM_AUDIO);
%     MS_powerspectrum_noise_correction(ax(4,3),A_pre(:,3),A_pre([1:noiseIdx,end-noiseIdx:end],3),savedSignals{i,2}.fs,pt_acc,ovl,FLIM_AUDIO);
%     
%     % Format Plots
